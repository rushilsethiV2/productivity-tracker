import { Dumbbell, CheckSquare, Calendar, ChevronRight, ChevronLeft, LayoutDashboard, BookOpen, Database, Waves } from 'lucide-react';
...
export default function Sidebar() {
  ...
  const apps = [
    ...
    { id: 'notes', name: 'Notes', icon: BookOpen, color: 'cyan' },
  ];
  ...
  return (
    <>
        ...
        <div className="flex flex-col h-full p-4 w-full">
          <div className="mb-8 mt-4 overflow-hidden">
            <div className="flex items-center gap-3 mb-2">
              <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 via-purple-500 to-cyan-500 flex items-center justify-center">
                <Waves className="w-6 h-6 text-white" />
              </div>
              {isExpanded && (
                <div className="animate-fadeIn">
                  <h2 className="text-lg font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-cyan-400 bg-clip-text text-transparent whitespace-nowrap">
                    Flow State
                  </h2>
                </div>
              )}
          ...
          <div className="mt-auto pt-4 space-y-3 border-t border-[rgb(var(--border))]">
            <button
              onClick={() => navigate('/auth')}
              className={`w-full flex items-center gap-4 p-3 rounded-xl transition-all relative group ${
                location.pathname.startsWith('/auth')
                  ? 'bg-gradient-to-r from-purple-500 to-blue-500 text-white shadow-lg'
                  : 'bg-[rgb(var(--background))] border border-[rgb(var(--border))] hover:border-purple-500'
              }`}
              title={!isExpanded ? 'Data & Sync' : ''}
            >
              <div
                className={`flex-shrink-0 ${
                  location.pathname.startsWith('/auth') ? '' : 'group-hover:scale-110'
                } transition-transform`}
              >
              ...
              {isExpanded && (
                <div className="text-left overflow-hidden animate-fadeIn">
                  <h3 className="font-semibold text-sm whitespace-nowrap">Data & Sync</h3>
                </div>
              )}
              {location.pathname.startsWith('/auth') && !isExpanded && (
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-white rounded-r-full" />
              )}
            </button>
            <button
      ...
      {/* Mobile Data Manager Button - Fixed Top Right */}
      <button
        onClick={() => navigate('/auth')}
        className={`md:hidden fixed top-4 right-4 z-50 p-3 rounded-full transition-all shadow-lg ${
          location.pathname.startsWith('/auth')
            ? 'bg-gradient-to-r from-purple-500 to-blue-500 text-white'
            : 'bg-[rgb(var(--card))] border border-[rgb(var(--border))] text-gray-300 hover:text-white hover:border-purple-500'
        }`}
