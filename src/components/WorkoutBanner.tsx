...
import { getItem, setItem, getRestDay, setRestDay } from '../utils/yearStorage';
...
export default function WorkoutBanner({ onNavigate }: WorkoutBannerProps) {
  ...
  const checkResetTime = () => {
    const now = new Date();
    const lastResetTime = getItem('workout_reset_time');

    if (!lastResetTime) {
      setItem('workout_reset_time', now.toISOString());
      return;
    }
  ...
  const checkTodayCompletion = (loadedRoutines: Routine[], loadedWeekly: WeeklyRoutine[]) => {
    ...
    const isRestToday = getRestDay(today) || hasWeeklyRestDay;
