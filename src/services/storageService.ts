...
export function saveRoutines(routines: Routine[]): void {
  setItem(ROUTINES_KEY, routines);
}

export function loadRoutines(): Routine[] {
  const data = getItem(ROUTINES_KEY);
  return data || [];
}
...
export function loadSessions(): WorkoutSession[] {
  const data = getItem(SESSIONS_KEY);
  return data || [];
}
...
export function saveWeeklyRoutines(routines: WeeklyRoutine[]): void {
  setItem(WEEKLY_ROUTINES_KEY, routines);
}

export function loadWeeklyRoutines(): WeeklyRoutine[] {
  const data = getItem(WEEKLY_ROUTINES_KEY);
  return data || [];
}
